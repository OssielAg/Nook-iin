...
def analiza(r1,r2,roAng=(0.0,180.0),erMax=0.5,mor=15,accuracy=2):
    '''
    ...
    rOrd = sorted(resultados, key=lambda r : r[1])
    top="\t"
    for i in range(min(len(rOrd),10)):
        top=top+"{}° ".format(rOrd[i][0])
    print("+-"*30+"\nLos ángulos con los errores mínimos son:\n{}\n".format(top)+"+-"*30)
    return resultados,rOrd

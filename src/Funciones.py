...
def importLattice(name,prnt=True):
    ...
    '''
    errormsg = '''El archivo no existe o no tiene el formato soportado por el programa.
    -Debe proporcionar el nombre de un archivo vasp (sin incluir el '.vasp').
    -El formato debe corresponder a un formato POSCAR del tipo 'DIRECTO'.
    -No se reconocen archivos POSCAR del tipo 'Cartesian'.'''
    xyz = []
    try:
        # cargamos el nombre de la Red
        if prnt: print("Se leerá el archivo {}".format(name+".vasp"))
        ...
        if prnt: print("--Red creada a partir del archivo '{}'--".format(name+".vasp"))
        return leido
    except:
        print(errormsg)
        return None
